...
class Decoder(srd.Decoder):
    ...
    def putdigit(self, ss, es, digit, value):
        self.put(ss, es, self.out_ann, [self.ann_digit, ['Digit %d: %02X' % (digit, value)]])
        ...
        if ptype == 'DATA':
            ...
            if self.pos == 0:
                ...
                if self.addr >= 1 and self.addr <= 8:
                    ...
                    self.putwarn(self.addr_start, es, 'Unknown register %02X' % (self.addr))
            ...
            if self.cs_asserted:
                ...
                if self.pos == 1:
                    # Don't warn if pos=0 so that CS glitches don't appear as
                    # spurious warnings
